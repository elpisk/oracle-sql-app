...
export default function AdminDashboard() {
  ...
  const fetchStudents = useCallback(async () => {
    setLoadingAT(true); setAtError(false)
    try {
  ...
  const handleAnswer = (id: string) => {
    const text = answerMap[id]?.trim(); if (!text) return
  ...
  const handleDelete = (id: string) => {
    ...
    deleteInquiry(id); setInquiries(getInquiries())
  }
  ...
  const toggleHidden = (id: string) => {
    ...
    setHidden(next); setHiddenChapters(next)
  }
          ...
          {TABS.map(({ key, label, icon: Icon }) => (
            <button key={key} onClick={() => setTab(key)}
              className={`flex items-center gap-1.5 px-4 py-3 text-[13px] font-semibold transition-colors border-b-2 ${
                tab === key ? 'text-apple-blue border-apple-blue' : 'text-apple-secondary border-transparent hover:text-apple-text'
              }`}>
        ...
        {tab === 'stats' && (
          <div className="space-y-5 fade-in">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                { label: '등록 학습자',  value: `${totalStudents}명`,             color: '#0071E3' },
                { label: '퀴즈 응시',    value: `${quizRows.length}건`,           color: '#AF52DE' },
                { label: '퀴즈 평균',    value: quizRows.length ? `${avgScore}점` : '--', color: '#FF9500' },
                { label: '강의 완료',    value: `${lectureRows.length}건`,        color: '#34C759' },
            ...
            <div className="flex items-center justify-between">
              <h2 className="text-[15px] font-bold text-apple-text">학습자별 현황</h2>
              <button onClick={fetchStudents} disabled={loadingAT}
                className="flex items-center gap-1.5 text-[12px] text-apple-secondary hover:text-apple-blue transition-colors">
                <RefreshCw size={13} className={loadingAT ? 'animate-spin' : ''} /> 새로고침
              </button>
            </div>
            ...
            {!atError && students && totalStudents === 0 && (
              <div className="bg-white rounded-apple-xl shadow-apple text-center py-14 text-apple-secondary">
                <BarChart2 size={36} className="mx-auto mb-3 opacity-30" />
                <p className="text-[14px]">아직 학습 데이터가 없습니다.</p>
                <p className="text-[12px] mt-1">학습자가 강의·퀴즈·실습을 완료하면 여기에 표시됩니다.</p>
              </div>
            )}
            ...
            {studentSummary.length > 0 && (
              <div className="space-y-2">
                {studentSummary.map(s => (
                  <div key={s.email} className="bg-white rounded-apple-lg shadow-apple overflow-hidden">
                    {/* 요약 행 */}
                    <button
                      onClick={() => setExpanded(e => e === s.email ? null : s.email)}
                      className="w-full flex items-center gap-4 px-5 py-4 hover:bg-apple-gray-bg/50 transition-colors text-left">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="text-[14px] font-semibold text-apple-text">{s.name}</span>
                          <span className="text-[11px] font-semibold text-apple-blue bg-blue-50 px-2 py-0.5 rounded-full">{s.cohort}</span>
                        </div>
                        <span className="text-[12px] text-apple-secondary">{s.email}</span>
                      </div>
                      <div className="flex items-center gap-4 text-[12px] flex-shrink-0">
                        <span className="text-apple-secondary">강의 <b className="text-apple-text">{s.lectureCount}</b></span>
                        <span className="text-apple-secondary">퀴즈 <b className="text-apple-text">{s.quizCount}</b></span>
                        <span className="text-apple-secondary">실습 <b className="text-apple-text">{s.practiceCount}</b></span>
                        {s.avgScore !== null && (
                          <span className={`font-bold ${s.avgScore >= 80 ? 'text-apple-green' : s.avgScore >= 60 ? 'text-apple-orange' : 'text-red-500'}`}>
                            평균 {s.avgScore}점
                          </span>
                        )}
                      </div>
                    ...
                    {expandedEmail === s.email && (
                      <div className="border-t border-apple-border px-5 py-4 space-y-4 bg-apple-gray-bg/30">
                        ...
                        {/* 강의 완료 이력 */}
                        {s.myLectures.length > 0 && (
                          <div>
                            <p className="text-[12px] font-bold text-apple-text mb-2">강의 완료 이력</p>
                            <div className="space-y-1.5">
              ...
              {(['all', 'pending', 'answered'] as const).map(f => (
                <button key={f} onClick={() => setInqFilter(f)}
                  className={`px-3 py-1.5 rounded-full text-[12px] font-semibold transition-colors ${
                    inqFilter === f ? 'bg-apple-blue text-white' : 'bg-white text-apple-secondary border border-apple-border hover:border-apple-blue'
                  }`}>
                ...
                <div key={inq.id} className="bg-white rounded-apple-xl shadow-apple p-5 space-y-3">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      <button onClick={() => handleDelete(inq.id)} className="p-1 text-apple-tertiary hover:text-red-500 transition-colors">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </div>
                  {(inq.authorName || inq.authorEmail) && (
                    <div className="flex items-center gap-2 bg-apple-gray-bg rounded-apple px-3 py-2">
                      ...
                      {inq.authorEmail && <span className="text-[12px] text-apple-secondary">{inq.authorEmail}</span>}
                      {inq.authorCohort && (
                  ...
                  <p className="text-[14px] text-apple-text leading-relaxed bg-apple-gray-bg rounded-apple px-4 py-3">{inq.content}</p>
                  {inq.answer && (
                  ...
                  <div className="flex gap-2">
                    <textarea value={answerMap[inq.id] ?? ''} rows={2}
                    ...
                    <button onClick={() => handleAnswer(inq.id)} disabled={!answerMap[inq.id]?.trim()}
                      className="px-4 py-2 rounded-apple bg-apple-blue text-white text-[13px] font-semibold hover:bg-apple-blue-dark disabled:opacity-40 flex items-center gap-1.5 flex-shrink-0 self-end transition-colors">
                      <Send size={13} />{inq.answer ? '수정' : '답변'}
                    </button>
                  </div>
                  <p className="text-[11px] text-apple-tertiary">{new Date(inq.createdAt).toLocaleString('ko-KR')}</p>
                </div>
              )
            })}
          </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="relative">
                <select value={selChapter} onChange={e => setSelChapter(e.target.value)}
                  className="appearance-none h-10 rounded-apple border border-apple-border pl-4 pr-8 text-[13px] text-apple-text bg-white focus:outline-none focus:border-apple-blue">
              ...
              <div className="flex rounded-apple overflow-hidden border border-apple-border">
                {(['quiz', 'practice'] as const).map(t => (
                  <button key={t} onClick={() => setQType(t)}
                    className={`px-4 h-10 text-[13px] font-semibold transition-colors ${qType === t ? 'bg-apple-blue text-white' : 'bg-white text-apple-secondary hover:text-apple-text'}`}>
                          ...
                          <div key={q.id} className="bg-white rounded-apple-lg shadow-apple p-4">
                            ...
                            <div className="mt-2 space-y-1">
                              {q.options.map((opt, oi) => (
                                <p key={oi} className={`text-[12px] px-3 py-1.5 rounded ${showAnswer[q.id] && oi === q.correctAnswer ? 'bg-green-50 text-apple-green font-semibold' : 'text-apple-secondary'}`}>
                  ...
                  return (
                    <div key={grp}>
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-[13px] font-bold text-apple-text">그룹 {grp}: {problems[0]?.groupTitle}</span>
                      </div>
                      <div className="space-y-2">
                        {problems.map((p, i) => (
                          <div key={p.id} className="bg-white rounded-apple-lg shadow-apple p-4 space-y-2">
                            ...
                            <pre className="bg-[#1D1D1F] text-[#A8FF78] text-[12px] rounded-apple p-3 overflow-x-auto whitespace-pre-wrap">{p.sql}</pre>
                            {p.result && <p className="text-[12px] text-apple-secondary"><span className="font-semibold text-apple-text">결과: </span>{p.result}</p>}
                            <div className="text-[12px] bg-yellow-50 border border-yellow-200 rounded-apple px-3 py-2">
                  ...
                  return (
                    <div key={ch.id} className="flex items-center px-5 py-3.5 gap-4">
                      <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0"
                        ...
                        <span className="text-[12px] font-bold" style={{ color: hasContent ? '#0071E3' : '#8E8E93' }}>{ch.number}</span>
                      </div>
                      <div className="flex-1 min-w-0">
                      ...
                      <button onClick={() => hasContent && toggleHidden(ch.id)} disabled={!hasContent}
                        className={`flex items-center gap-1.5 text-[12px] font-semibold transition-colors ${
                          !hasContent ? 'text-apple-tertiary cursor-not-allowed'
                          : isHidden ? 'text-apple-tertiary hover:text-apple-orange'
                          : 'text-apple-green hover:text-apple-orange'
                        }`}>
                        {isHidden ? <><ToggleLeft size={20} className="text-apple-tertiary" />숨김</> : <><ToggleRight size={20} className="text-apple-green" />공개</>}
                      </button>
                    </div>
                  )
                })}
